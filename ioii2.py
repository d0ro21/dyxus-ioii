...
if uploaded_file is not None:
    ...
    if st.button("🚀 Otimizar Frota (Correr Gurobi)", type="primary"):
        with st.spinner("A calcular a rede ótima com modelo Não-Linear..."):
            ...
            env.start()
            model = gp.Model("Burrito_Game_Marketing_NonLinear", env=env)
            
            # PARÂMETRO CHAVE: Permitir multiplicações entre variáveis de decisão (Não-Convexo)
            model.Params.NonConvex = 2

            Y = model.addVars(J, TIPOS, vtype=GRB.BINARY, name="Y")
            X = model.addVars(I, J, vtype=GRB.BINARY, name="X")

            # Variável do Marketing (sem linearização auxiliar)
            M = model.addVars(I, vtype=GRB.INTEGER, lb=0, ub=max_campanhas, name="Marketing")

            # Função Objetivo (Multiplicação Direta: X * (Procura + Marketing))
            lucro_var = gp.quicksum((r - k_ing) * (X[i, j] * (a[i, j] + aumento_pct * a[i, j] * M[i])) for i in I for j in J if (i, j) in a)
            ...
            # Restrição de Capacidade (Multiplicação Direta: X * (Procura + Marketing))
            for j in J:
                model.addConstr(
                    gp.quicksum(X[i, j] * (a[i, j] + aumento_pct * a[i, j] * M[i]) for i in I if (i, j) in a) 
                    <= gp.quicksum(CAPACIDADE[t] * Y[j, t] for t in TIPOS)
                )
            ...
            if model.Status == GRB.OPTIMAL:
                ...
                # Cálculos
                total_burritos = sum(X[i, j].X * (a[i, j] + aumento_pct * a[i, j] * M[i].X) for i in I for j in J if (i, j) in a and X[i, j].X > 0.5)
                ...
                with col_tab1:
                    ...
                    st.dataframe(pd.DataFrame([{
                        "Camião": j, "Tipo": t, "Cap.": CAPACIDADE[t], 
                        "Vendas": int(sum(X[i, j].X * (a[i, j] + aumento_pct * a[i, j] * M[i].X) for i in I if (i, j) in a and X[i, j].X > 0.5)), 
                        "Custo (€)": CUSTO_FIXO[t]
                    } for j, t in opened_trucks]), use_container_width=True, hide_index=True)
                with col_tab2:
                    st.subheader("🏢 Alocação de Edifícios")
                    st.dataframe(pd.DataFrame([{
                        "Edifício": i, "Camião": j, 
                        "Tipo": next(t for t in TIPOS if Y[j, t].X > 0.5), 
                        "Burritos": int(X[i, j].X * (a[i, j] + aumento_pct * a[i, j] * M[i].X))
                    } for i in I for j in J if (i, j) in a and X[i, j].X > 0.5]), use_container_width=True, hide_index=True)
